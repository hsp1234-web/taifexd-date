...
class DatabaseLoader:
    ...
    def load_parquet(self, table_name: str, parquet_file_path: str):
        ...
        try:
            ...
            # Get the count of actual rows in the table after insert (more reliable for idempotency)
            # However, this doesn't tell how many rows were *newly* inserted by THIS operation.
            # DuckDB's INSERT...ON CONFLICT... does not directly return the number of affected/inserted rows.
            # To get newly inserted rows, one might need a more complex approach (e.g., temp table, diff).
            # For now, we log total rows and rely on ON CONFLICT.

            # A simple way to check if any rows were *changed* by this operation is not straightforward
            # with DO NOTHING. We are assuming idempotency means no error and state is consistent.
            # The count of newly inserted rows isn't easily available.
            # We can log the total count in table.

            # Get current total row count in the table
            total_rows_in_table_query = f"SELECT COUNT(*) FROM \"{table_name}\";"
            total_rows = self.connection.execute(total_rows_in_table_query).fetchone()[0]

            # To know how many rows were *attempted* to be inserted from the Parquet file:
            rows_in_parquet_query = f"SELECT COUNT(*) FROM read_parquet('{parquet_file_path}')"
            attempted_insert_count = self.connection.execute(rows_in_parquet_query).fetchone()[0]

            self.logger.info(
                f"✅ 資料載入操作完成於資料表 '{table_name}'。"
                f" Parquet 檔案 '{parquet_file_path}' 中包含 {attempted_insert_count} 筆記錄。"
                f" 資料表 '{table_name}' 現在總共有 {total_rows} 筆記錄。"
            )
